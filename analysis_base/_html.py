...
<body>
<h3>Analysis documents for %(upper)s</h3>""" 
# fix the next higher menu
#<h3><a %(models)s/%(upper)s</h3>""" 
...
def table_menu(max_cols=20):
    ...
    for m in models:
        t = os.path.split(m)[0]
        mname = t.split(os.sep)[-2]
        dnames = [x.split(os.sep)[-2] for x in glob.glob(t+'/*/index.html')]
    ...
    # now edit the HTML to include links
...
class HTMLindex():
    ...
    def __init__(self, folder=None):
        ...
        w = folder if folder is not None else os.getcwd().split(os.path.sep) 
        self.model = w[-1] #'/'.join(w[-2:])
        self.upper = w[-2]+'/'
        #print self.upper, self.model
        ...
        plot_folders = [x.split(os.sep)[1] for x in sorted(glob.glob('plots/*/index.html'))]
        assert len(plot_folders)>0, 'No folders with plots found'
    ...
    def update_top(self, filename='../plot_index.html'):
        """ replace the top menu, containing each of the models
        """
        def parse_path(x): 
            'return relative path, model name'
            t = x.split(os.path.sep)
            #print 'parsing path {}'.format(t)
            return  '/'.join(t[1:]) , t[1]
        def parse_model(x):
            return '<a href="%s?skipDecoration"> %s </a>' %(parse_path(x) )
            
        def model_comment(x):
            """ extract the comment field from the json describing this model
            """
            a,b=parse_path(x)
            filename ='../'+b+'/config.json'
            assert os.path.exists(filename), 'Configuration file {} not found'.format(filename)
            try:
                return json.load(open(filename))['comment']
            except:
                return '(no comment)'
        ...
        self.models='<a href="../plot_index.html?skipDecoration">models</a>'
        self.series = os.getcwd().split(os.sep)[-2]
